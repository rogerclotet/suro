...
function Exclusion({
  ...
  async function handleDelete() {
    try {
      ...
      toast.success("Exclusió eliminada");
    } catch (error) {
      ...
      toast.error(
        "No s'ha pogut eliminar l'exclusió, torna-ho a provar més tard",
      );
    }
  }
  ...
  return (
    ...
    >
      ...
      <ModalAction
        title={`Eliminar exclusió: ${exclusion.map((participant) => participants.find((p) => p.id === participant)?.user.name).join(", ")}`}
        description="Estàs segur que vols eliminar aquesta exclusió? Aquesta acció no es pot desfer."
        actionText="Eliminar"
