(
  sshpass -p $SSH_PASSWORD ssh $SSH_USERNAME@$SSH_IP -o StrictHostKeyChecking=no <<-EOF
    ...
    docker-compose -p familia -f docker-compose.yml -f docker-compose.prod.yml stop
    DB_NAME=$DB_NAME DB_USER=$DB_USER DB_PASSWORD=$DB_PASSWORD \
        SECRET_KEY=$SECRET_KEY STATIC_ROOT=$STATIC_ROOT \
        docker-compose -p familia -f docker-compose.yml -f docker-compose.prod.yml up -d
    STATIC_ROOT=$STATIC_ROOT docker-compose -p familia -f docker-compose.yml -f docker-compose.prod.yml \
