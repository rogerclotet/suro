...
`apps/web/CHANGELOG.md` is the curated, user-facing changelog — **not** a full history. With every commit, update it **only if** the change is an important fix or a user-facing change (new feature, visible behavior change, notable bugfix). Skip it for refactors, internal/infra changes, dependency bumps, tests, or docs.
...
- No manual codegen needed — `src/data/changelog.generated.ts` is regenerated from `apps/web/CHANGELOG.md` by the `pre*` hooks (`predev`/`prebuild`/`pretest`/`pretypecheck`). Run `pnpm changelog:generate` to preview.
...
CI is split across focused workflows (so each Actions run stays a small graph):

| Workflow | When |
| --- | --- |
| `ci.yml` (**CI**) | Every PR / push to `main`: quality checks, web PR compile-check, release tag |
| `deploy-web.yml` | Push to `main` when web/backend paths change → Docker image + Convex + SSH prod |
| `mobile-release.yml` | Push to `main` when the native release gate passes → EAS + store submit; also **Actions → Mobile release → Run workflow** (skips the gate, `main` only) |
| `play-listing.yml` | Push to `main` when Play listing text/images change |
...
- **Mobile builds**: on `main`, when the release gate passes, **EAS cloud builds** run in the **Mobile release** workflow (`mobile_eas_build_android` / `mobile_eas_build_ios`) in parallel — they build on Expo's infra (no Android/iOS toolchain on the runner), wait for each build to finish, download the store-ready `.aab`/`.ipa`, and then `mobile_play_store_submit` / `mobile_app_store_submit` publish them. **Android** goes to the Play **production** track at full rollout (`fastlane android release`). **iOS** runs `eas submit` (binary) → `eas metadata:push` (listing text from `store.config.json`) → `fastlane ios submit_for_review`, which waits for Apple to finish processing the build, attaches it, re-uploads screenshots when `store/apple/screenshots/**` changed in that push, and submits for review with `automatic_release` so approval ships it. That fastlane lane runs with `skip_binary_upload`, so it only talks to the App Store Connect API and needs no iTMSTransporter on the Linux runner. Play listing text/images push independently via the **Play listing** workflow whenever listing metadata paths change (same `play-console` concurrency group as the binary submit, so fastlane never overlaps). To force a full build+submit without a gate-passing push: Actions → **Mobile release** → Run workflow (from `main`; optional `push_screenshots`). A separate **Mobile submit** workflow remains for re-submission of a specific already-built commit (`ios_skip_upload` retries just the metadata + review submission; `push_screenshots` forces a screenshot re-upload). Neither store needs a console visit — only their own review gates the release. The `production` profile auto-increments the remote version on every build, so each gated release bumps the build number. You can still build locally via the `build:*` scripts in `apps/mobile` — `pnpm --filter mobile build:android:preview` (dev-Convex `.apk`), `build:android:release` (store-ready `.aab`), `build:ios:release` (store-ready `.ipa`). Signing uses EAS-managed credentials (`eas login` / `EXPO_TOKEN`); public build env (`EXPO_PUBLIC_*`) is baked per profile in `apps/mobile/eas.json`. Store publishing (EAS Submit profiles, listing metadata for both stores, screenshots, console declarations) lives in `apps/mobile/store/` — see `apps/mobile/store/README.md`. See `apps/mobile/README.md` for build prerequisites.

A version bump on `main` (root `package.json`, in lockstep with the top `apps/web/CHANGELOG.md` entry) also runs `release_tag` → `scripts/publish-release.mjs`, which tags the commit `v<version>` and publishes a GitHub release with that changelog entry as its notes.
