...
- **Mobile builds**: run on `main` only and **only when mobile-relevant paths change** (`apps/mobile/`, `packages/backend/`, lockfile, CI config). `build_mobile_android_preview` (dev-Convex internal `.apk`) and `build_mobile_android_release` (store-ready `.aab`) build locally on the runner via `eas build --local`; `build_mobile_ios_release` (store-ready `.ipa`) builds on EAS cloud — currently commented out until an Apple Developer membership + iOS credentials exist on EAS. Artifacts attach to the jobs. Signing uses EAS-managed credentials (authed by `EXPO_TOKEN`); there is no `eas submit` step yet. Public build env (`EXPO_PUBLIC_*`) lives in `apps/mobile/eas.json` per profile — the EAS local-build sandbox does not inherit CI job env. See `apps/mobile/README.md` for the one-time credential setup.

CI variables to set: `CONVEX_DEPLOY_KEY` (prod deploy key), `NEXT_PUBLIC_CONVEX_URL` (prod), `NEXT_PUBLIC_CONVEX_URL_PREVIEW` (dev), `EXPO_TOKEN` (EAS access token, protected + masked), plus the existing PostHog + registry/SSH vars.
