...
/**
 * Expense math for the offline overlay, ported verbatim from the backend's
 * `packages/backend/convex/model/expenses.ts` so an offline pot detail screen
 * recomputes balances/settle-up the same way the server would. Kept in sync by
 * hand (both are small, pure, and covered by tests). All amounts are integer
 * cents.
 */
...
/**
 * Net balance per member, in cents. Positive = owed money; negative = owes.
 * Direct spendings (`to` set) move the full amount; split spendings (`to`
 * unset) divide equally, with the rounding cents going to the earliest in
 * [payer, ...others sorted by id] so balances sum to exactly zero.
 */
 ...
 * creditor until everyone is square. Not minimal, but matches the backend.
 */
