...
export const spendings = createTable(
  ...
  (s) => ({
    projectIdIdx: index("spending_projectId_idx").on(s.projectId),
    potIdIdx: index("spending_potId_idx").on(s.potId),
    fromIdx: index("spending_from_idx").on(s.from),
    toIdx: index("spending_to_idx").on(s.to),
  }),
);
