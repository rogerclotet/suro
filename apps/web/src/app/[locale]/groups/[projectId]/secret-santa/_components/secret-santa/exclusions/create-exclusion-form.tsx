...
export default function CreateExclusionForm({
  ...
  async function onSubmit(data: v.InferOutput<typeof exclusionSchema>) {
    ...
    try {
      ...
      if (result?.error) {
        toast.error(
          t("exclusionCreateErrorWithMessage", { error: result.error }),
        );
      } else {
        toast.success(t("exclusionCreateSuccess"));
      }
      ...
      toast.error(t("exclusionCreateError"));
    }
  }
